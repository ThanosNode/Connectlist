...
// Define key states we want to focus on
const keyStates = [
  "California", "New York", "Texas", "Florida", "Georgia",
  "Illinois", "Pennsylvania", "Ohio", "Washington", "Michigan"
];
...
function createListingObject(userId, type, subcategory, state, city) {
...
async function insertKeyListings() {
  console.log('Starting to insert sample listings for key states...');
    ...
    for (const state of keyStates) {
      ...
      for (const city of cities) {
        ...
        for (const [type, subcategories] of Object.entries(categories)) {
          // For each subcategory, create a sample listing
          for (const subcategory of subcategories) {
            const listing = createListingObject(userId, type, subcategory, state, city);
        ...
        console.log(`Completed listings for ${city}, ${state}`);
      }
      console.log(`Completed all cities for ${state}`);
    }
...
// Run the script
insertKeyListings().catch(console.error);